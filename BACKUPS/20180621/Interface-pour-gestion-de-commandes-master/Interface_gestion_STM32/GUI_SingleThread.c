...
void GUIThread (void const *argument) {
  ...
  GUI_Clear();
	...
	UART_transfert("AT\r\n",4);
		...
		GUI_DispStringAt(texte , 420, 127);
		GUI_Exec();
	UART_transfert("AT+ROLE=1\r\n", 11);
	
		sprintf (texte, "X=3");
		GUI_DispStringAt(texte , 420, 127);
		GUI_Exec();
	UART_transfert("AT+INIT\r\n", 9);*/
	
  while (1) {
		drawImages((productList+(itemClicked + (4*oldPage))));									//Draws the images on the UI
		if(productDrawned == 0) {
...
int main (void) {
	...
	sprintf(name,"Tomates");
		strcopy(name,(productList)->name);    //Init Product Name
	(productList)->image = &bmtomate;
	(productList)->quantity = 5;
	sprintf(loca,"2°Allee, 5°Panier");
		strcopy(loca,(productList+i)->location);//Init Product Location		
	(productList)->status = 0;            //Init Product Status	
	
	sprintf(name,"Pommes de terre");
		strcopy(name,(productList+1)->name);    //Init Product Name
	(productList)->image = &bmtomate;
	(productList+1)->quantity = 15;
	sprintf(loca,"1°Allee, 3°Panier");
		strcopy(loca,(productList+1)->location);//Init Product Location		
	(productList+1)->status = 0;            //Init Product Status	
	
	sprintf(name,"Poireaux");
		strcopy(name,(productList+2)->name);    //Init Product Name
	(productList)->image = &bmtomate;
	(productList+2)->quantity = 2;
	sprintf(loca,"4°Allee, 8°Panier");
		strcopy(loca,(productList+2)->location);//Init Product Location		
	(productList+2)->status = 0;            //Init Product Status	
	
	sprintf(name,"Courgettes");
		strcopy(name,(productList+3)->name);    //Init Product Name
	(productList)->image = &bmtomate;
	(productList+3)->quantity = 3;
	sprintf(loca,"3°Allee, 4°Panier");
		strcopy(loca,(productList+3)->location);//Init Product Location		
	(productList+3)->status = 0;            //Init Product Status	
...
void drawImages(struct product* prod) {
	...
	GUI_DrawBitmap(prod->image, ItemImag[0], ItemImag[1]); //Displays the image
...
void drawProduct(struct product* prod) {
	//const GUI_BITMAP *bmp = prod->image;
...
void makeUI() {
	...
	drawImages((productList+(itemClicked + (4*oldPage))));//Draw Bitmaps on the screen
	drawList();
}
