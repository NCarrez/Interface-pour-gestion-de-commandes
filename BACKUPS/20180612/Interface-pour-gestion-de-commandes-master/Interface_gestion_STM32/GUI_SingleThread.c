...
void UART_send(char*,int);
void UART_receive(char[],int*);
//Drawing
void cleanList(void);
void clearUI(void);
...
void Init_BT() {
	char name[50];
	int baudRate[10] = {4800, 9600, 19200, 38400, 57600, 115200, 23400, 460800, 921600, 1382400},i;
	for(i=0;i<10;i++) {
...
void UART_send(char* string, int size) {
	while(Driver_USART6.GetStatus().tx_busy == 1); // attente buffer TX vide
	Driver_USART6.Send(string,size);
}
void UART_receive(char receive_tab[], int *size) {
	//uint8_t tab[100];
	int c=0;
	while(Driver_USART6.GetStatus().rx_busy==1);
	Driver_USART6.Receive(receive_tab,4);
	//
	//strcopy(tab, receive_tab);
	*size = strlen(receive_tab);
}
